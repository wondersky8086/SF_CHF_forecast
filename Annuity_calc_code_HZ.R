...
calculate_annuity_price_female <- function(P, N, T)
{
    ...
    for(t in 1:T)
    {
        ...
        if(P == "c")
        {
            SP[t] <- SV_H_female_forecast[N - 64 + t, t+1]/SV_H_female_forecast[N - 64, 1]
        }
        else if (P == "l")
        {
            SP[t] <- SV_H_female_forecast_lower[N - 64 +t, t+1]/SV_H_female_forecast_lower[N - 64, 1]
        }
        else if (P == "h")
        {
            SP[t] <- SV_H_female_forecast_upper[N - 64 +t, t+1]/SV_H_female_forecast_upper[N - 64, 1]
        }
    }
...
calculate_annuity_price_male <- function(P, N, T)
{
  ...
  for(t in 1:T)
  {
    ...
    if(P == "c")
    {
      SP[t] <- SV_H_male_forecast[N - 64 + t, t+1]/SV_H_male_forecast[N - 64, 1]
    }
    else if (P == "l")
    {
      SP[t] <- SV_H_male_forecast_lower[N - 64 +t, t+1]/SV_H_male_forecast_lower[N - 64, 1]
    }
    else if (P == "h")
    {
      SP[t] <- SV_H_male_forecast_upper[N - 64 +t, t+1]/SV_H_male_forecast_upper[N - 64, 1]
    }
  }
  # Calculate the annuity payment for year t
  AP <- 1  # Annuity payment Adjust if needed
